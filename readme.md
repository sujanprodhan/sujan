# 👋 Hi, I'm Sujan

### Lead Software Engineer • 13+ Years Experience • Bangladesh

I am a passionate **Lead Software Engineer** with over **13 years of experience** in designing, developing, and scaling high-quality software solutions.

I specialize in **React, Next.js, Node.js, Express.js, MySQL**, and modern JavaScript ecosystems.  
My journey includes building large-scale applications, leading engineering teams, and delivering products used by millions.

---

## 🚀 About Me

- 💼 Currently working as **Lead Software Engineer at Arogga Ltd**, a leading healthcare-tech platform in Bangladesh.  
- 🌍 Worked with teams from **Japan, USA, Vietnam, and Bangladesh**.  
- 🧠 Expert in **frontend architecture**, **backend APIs**, and **scalable system design**.  
- 🧑‍🏫 Strong experience in **team leadership**, code reviews, mentoring, and aligning engineering with business goals.  
- 🛠 Passionate about solving real-world problems through clean and efficient technology.

---

## 💼 Career History
-----------------------------------------------------------------------------------------
| Period       | Position                            | Company                            |
|--------------|-------------------------------------|------------------------------------|
| 2023 – Present | Lead Software Engineer            | Arogga Ltd., Bangladesh            |
| 2019– 2023   | Principal Software Engineer / Lead | BJIT Limited, Bangladesh           
| 2016 – 2019  | Senior Software Engineer            |  Doodle i Inc, Banglalion  
| 2013 – 2016  | Software Engineer                   | SSIT SOLUTION 
---

## 💡 What I Do

### **Frontend Engineering**
- React, Next.js, JavaScript, TypeScript  
- Tailwind CSS, Redux, CoreUI, UI/UX best practices  
- Responsive, optimized, production-ready interfaces  

### **Backend Development**
- Node.js, Express.js  
- REST API design, authentication, caching  
- MySQL, optimized queries, database architecture  

### **Architecture & Leadership**
- System design & scalability  
- Microservices & modular codebases  
- Leading engineering teams & code review  
- Performance optimization  

---

## 🔧 Technologies & Tools

### **Languages & Frameworks**
- JavaScript, TypeScript  
- React, Next.js  
- Node.js, Express.js  
- PHP8, PHP ( CodeIgniter, CakePHP, Symfony, Laravel, Wordpress )
- MySQL, MSSQL, MongoDB, GraphQL
- CSS3, Tailwind CSS, Bootstrap, SCSS

### **Tools & Platforms**
- Git, GitHub, Docker  
- Vercel, Nginx, cPanel  
- CI/CD pipelines  
- Linux & Ubuntu servers  

---

## 🌟 Featured Skills

- 🚀 Building scalable and high-performance systems  
- 🎨 Designing clean UI & UX with modern frontend tools  
- 🔐 Secure and optimized backend services  
- 🧩 Problem solving & system architecture  
- 👨‍💻 Leading developers with guidance and clarity  
- 📈 Delivering business-aligned technical solutions  

---

## 📬 Contact Me

- 📧 **Email:** sujanitbd@gmail.com  
- 💼 **LinkedIn:** https://www.linkedin.com/in/muhammad-sujan/  
- 🌐 **Portfolio:** https://github.com/sujanprodhan  

---

## ⭐ If you like my work  
Feel free to ⭐ star my repositories and connect with me!

